...
contract Basispointa {
        ...
        returns (uint256 meanBps, uint256 peakBps, uint256 floorBps, uint256 samples)
    {
        ...
        if (floorBps == type(uint256).max) floorBps = 0;
    }

    function laneBpsBundle1(uint256 startLaneId) external view returns (uint256[3] memory bundle) {
        for (uint256 j = 0; j < 3; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle2(uint256 startLaneId) external view returns (uint256[4] memory bundle) {
        for (uint256 j = 0; j < 4; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle3(uint256 startLaneId) external view returns (uint256[5] memory bundle) {
        for (uint256 j = 0; j < 5; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle4(uint256 startLaneId) external view returns (uint256[6] memory bundle) {
        for (uint256 j = 0; j < 6; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle5(uint256 startLaneId) external view returns (uint256[7] memory bundle) {
        for (uint256 j = 0; j < 7; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle6(uint256 startLaneId) external view returns (uint256[8] memory bundle) {
        for (uint256 j = 0; j < 8; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle7(uint256 startLaneId) external view returns (uint256[2] memory bundle) {
        for (uint256 j = 0; j < 2; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle8(uint256 startLaneId) external view returns (uint256[3] memory bundle) {
        for (uint256 j = 0; j < 3; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle9(uint256 startLaneId) external view returns (uint256[4] memory bundle) {
        for (uint256 j = 0; j < 4; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle10(uint256 startLaneId) external view returns (uint256[5] memory bundle) {
        for (uint256 j = 0; j < 5; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle11(uint256 startLaneId) external view returns (uint256[6] memory bundle) {
        for (uint256 j = 0; j < 6; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle12(uint256 startLaneId) external view returns (uint256[7] memory bundle) {
        for (uint256 j = 0; j < 7; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle13(uint256 startLaneId) external view returns (uint256[8] memory bundle) {
        for (uint256 j = 0; j < 8; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle14(uint256 startLaneId) external view returns (uint256[2] memory bundle) {
        for (uint256 j = 0; j < 2; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle15(uint256 startLaneId) external view returns (uint256[3] memory bundle) {
        for (uint256 j = 0; j < 3; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle16(uint256 startLaneId) external view returns (uint256[4] memory bundle) {
        for (uint256 j = 0; j < 4; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle17(uint256 startLaneId) external view returns (uint256[5] memory bundle) {
        for (uint256 j = 0; j < 5; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle18(uint256 startLaneId) external view returns (uint256[6] memory bundle) {
        for (uint256 j = 0; j < 6; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle19(uint256 startLaneId) external view returns (uint256[7] memory bundle) {
        for (uint256 j = 0; j < 7; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle20(uint256 startLaneId) external view returns (uint256[8] memory bundle) {
        for (uint256 j = 0; j < 8; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle21(uint256 startLaneId) external view returns (uint256[2] memory bundle) {
        for (uint256 j = 0; j < 2; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle22(uint256 startLaneId) external view returns (uint256[3] memory bundle) {
        for (uint256 j = 0; j < 3; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    ...
    function laneBpsBundle35(uint256 startLaneId) external view returns (uint256[2] memory bundle) {
        for (uint256 j = 0; j < 2; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle36(uint256 startLaneId) external view returns (uint256[3] memory bundle) {
        for (uint256 j = 0; j < 3; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle37(uint256 startLaneId) external view returns (uint256[4] memory bundle) {
        for (uint256 j = 0; j < 4; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle38(uint256 startLaneId) external view returns (uint256[5] memory bundle) {
        for (uint256 j = 0; j < 5; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle39(uint256 startLaneId) external view returns (uint256[6] memory bundle) {
        for (uint256 j = 0; j < 6; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function laneBpsBundle40(uint256 startLaneId) external view returns (uint256[7] memory bundle) {
        for (uint256 j = 0; j < 7; ++j) {
            uint256 laneId = startLaneId + j;
            bundle[j] = lanes[laneId].asset == address(0) ? 0 : laneLastBps[laneId];
        }
    }
    function rollingMeanBundle1(uint256 startLaneId) external view returns (uint256[4] memory means) {
        for (uint256 j = 0; j < 4; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
    ...
    function rollingMeanBundle2(uint256 startLaneId) external view returns (uint256[5] memory means) {
        for (uint256 j = 0; j < 5; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle3(uint256 startLaneId) external view returns (uint256[6] memory means) {
        for (uint256 j = 0; j < 6; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle4(uint256 startLaneId) external view returns (uint256[7] memory means) {
        for (uint256 j = 0; j < 7; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle5(uint256 startLaneId) external view returns (uint256[3] memory means) {
        for (uint256 j = 0; j < 3; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle6(uint256 startLaneId) external view returns (uint256[4] memory means) {
        for (uint256 j = 0; j < 4; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle7(uint256 startLaneId) external view returns (uint256[5] memory means) {
        for (uint256 j = 0; j < 5; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle8(uint256 startLaneId) external view returns (uint256[6] memory means) {
        for (uint256 j = 0; j < 6; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle9(uint256 startLaneId) external view returns (uint256[7] memory means) {
        for (uint256 j = 0; j < 7; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle10(uint256 startLaneId) external view returns (uint256[3] memory means) {
        for (uint256 j = 0; j < 3; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle11(uint256 startLaneId) external view returns (uint256[4] memory means) {
        for (uint256 j = 0; j < 4; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle12(uint256 startLaneId) external view returns (uint256[5] memory means) {
        for (uint256 j = 0; j < 5; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle13(uint256 startLaneId) external view returns (uint256[6] memory means) {
        for (uint256 j = 0; j < 6; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle14(uint256 startLaneId) external view returns (uint256[7] memory means) {
        for (uint256 j = 0; j < 7; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle15(uint256 startLaneId) external view returns (uint256[3] memory means) {
        for (uint256 j = 0; j < 3; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle16(uint256 startLaneId) external view returns (uint256[4] memory means) {
        for (uint256 j = 0; j < 4; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle17(uint256 startLaneId) external view returns (uint256[5] memory means) {
        for (uint256 j = 0; j < 5; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function rollingMeanBundle18(uint256 startLaneId) external view returns (uint256[6] memory means) {
        for (uint256 j = 0; j < 6; ++j) {
            uint256 laneId = startLaneId + j;
            LaneSheet storage lane = lanes[laneId];
            means[j] = lane.asset == address(0)
    ...
    function healthScoreBundle7(uint256 startLaneId) external view returns (uint256[3] memory scores) {
        for (uint256 k = 0; k < 3; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle8(uint256 startLaneId) external view returns (uint256[4] memory scores) {
        for (uint256 k = 0; k < 4; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle9(uint256 startLaneId) external view returns (uint256[5] memory scores) {
        for (uint256 k = 0; k < 5; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle10(uint256 startLaneId) external view returns (uint256[6] memory scores) {
        for (uint256 k = 0; k < 6; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle11(uint256 startLaneId) external view returns (uint256[7] memory scores) {
        for (uint256 k = 0; k < 7; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle12(uint256 startLaneId) external view returns (uint256[2] memory scores) {
        for (uint256 k = 0; k < 2; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle13(uint256 startLaneId) external view returns (uint256[3] memory scores) {
        for (uint256 k = 0; k < 3; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle14(uint256 startLaneId) external view returns (uint256[4] memory scores) {
        for (uint256 k = 0; k < 4; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle15(uint256 startLaneId) external view returns (uint256[5] memory scores) {
        for (uint256 k = 0; k < 5; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle16(uint256 startLaneId) external view returns (uint256[6] memory scores) {
        for (uint256 k = 0; k < 6; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle17(uint256 startLaneId) external view returns (uint256[7] memory scores) {
        for (uint256 k = 0; k < 7; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle18(uint256 startLaneId) external view returns (uint256[2] memory scores) {
        for (uint256 k = 0; k < 2; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle19(uint256 startLaneId) external view returns (uint256[3] memory scores) {
        for (uint256 k = 0; k < 3; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function healthScoreBundle20(uint256 startLaneId) external view returns (uint256[4] memory scores) {
        for (uint256 k = 0; k < 4; ++k) {
            uint256 laneId = startLaneId + k;
            scores[k] = _laneHealthScoreInternal(laneId);
        }
    }
    function scanThresholdsTier1(address user, uint256 startId, uint256 limit)
        external
        view
    ...
    function scanThresholdsTier2(address user, uint256 startId, uint256 limit)
        external
        view
        returns (uint256[] memory failingIds)
    {
    ...
    function scanThresholdsTier3(address user, uint256 startId, uint256 limit)
        external
        view
        returns (uint256[] memory failingIds)
    {
    ...
    function scanThresholdsTier4(address user, uint256 startId, uint256 limit)
        external
        view
        returns (uint256[] memory failingIds)
    {
    ...
    function scanThresholdsTier5(address user, uint256 startId, uint256 limit)
        external
        view
        returns (uint256[] memory failingIds)
    {
    ...
    function scanThresholdsTier6(address user, uint256 startId, uint256 limit)
        external
        view
        returns (uint256[] memory failingIds)
    {
    ...
    function scanThresholdsTier7(address user, uint256 startId, uint256 limit)
        external
        view
        returns (uint256[] memory failingIds)
    {
    ...
    function scanThresholdsTier8(address user, uint256 startId, uint256 limit)
        external
        view
        returns (uint256[] memory failingIds)
    {
    ...
    function scanThresholdsTier9(address user, uint256 startId, uint256 limit)
        external
        view
        returns (uint256[] memory failingIds)
    {
    ...
    function scanThresholdsTier10(address user, uint256 startId, uint256 limit)
        external
        view
        returns (uint256[] memory failingIds)
    {
    ...
    function rankLaneByLastBps_2(uint256 candidateLaneId, uint256[] calldata peerLaneIds)
        external
        view
        returns (uint256 rank, uint256 candidateBps)
    {
    ...
    function rankLaneByLastBps_3(uint256 candidateLaneId, uint256[] calldata peerLaneIds)
        external
        view
        returns (uint256 rank, uint256 candidateBps)
    {
    ...
    function rankLaneByLastBps_4(uint256 candidateLaneId, uint256[] calldata peerLaneIds)
        external
        view
        returns (uint256 rank, uint256 candidateBps)
    {
    ...
    function rankLaneByLastBps_5(uint256 candidateLaneId, uint256[] calldata peerLaneIds)
        external
        view
        returns (uint256 rank, uint256 candidateBps)
    {
